...
class ConferencesTest {
  ...
  void contentCanBeNull() {
    Conferences confs = new Conferences(null, List.of());

    assertTrue(confs.empty());
    assertEquals("", confs.markdown());
  }

  @Test
  void contentCanBeBlank() {
    Conferences confs = new Conferences("   ", List.of());
  ...
  @Test
  void realContentWithoutCandidates() {
    Conferences confs =
        new Conferences(
            """
  ...
  @Test
  void realContentWithCandidates() {
    Conferences confs =
        new Conferences(
            """
      # Developers Conferences Agenda/List
      ...
      * 20-23: [JCON PEOPLE](https://jcon.one/) - Düsseldorf, Germany\s
    ...
    String markdown =
        confs
            .markdown()
            // s.matches(regex) ne fonctionne pas sur des multiline strings
            .replaceAll("\\n", " ");

    assertFalse(confs.empty());
    assertTrue(markdown.matches(".*4-6 octobre 2022 : .*Devoxx Morocco.*"));
    assertTrue(markdown.matches(".*6-7 octobre 2022 : .*Paris Web.*"));
    assertTrue(markdown.matches(".*15-16 novembre 2022 : .*Agile Tour Toulouse.*"));
    assertTrue(markdown.matches(".*19-20 janvier 2023 : .*Touraine Tech.*"));
  }

  @Test
  void contentWithInvalidMonth() {
    Conferences confs =
        new Conferences(
            """
      ## 2022
    ...
    assertFalse(confs.empty());
    assertTrue(markdown.matches(".*6-7 InvalidMonth 2022 : .*Paris Web.*"));
  }
}
